...
t2025_77_lines <- read_excel("~/Library/CloudStorage/OneDrive-Personal/Documents/R Studio/Schedule Calendars/2024-04-04 R 7-7 Calendar.xlsx", 
                             sheet = "CalendR Days 2025",
                             range = cell_cols(17:28))
...
for (i in 1:12) {
  ...
  line_data <- unlist(t2025_77_lines[i])
  line <- var_list[i]
  line_legend <- sub("_"," ",line)
  line_no <- ifelse(nchar(line) == 7, str_sub(line,-2), str_sub(line,-1))
  cal_title <- glue("2025 7&7-{line_no}")
  save_as <- glue("7_7_{line}.png")
...
imp.dates[t2024_77_lines$Line_2] <- "Line 2"
imp.dates[c(1,15,50,89,91,148,186,246,333,360)] <- "NJA Holidays"
